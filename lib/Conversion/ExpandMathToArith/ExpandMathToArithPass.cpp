 ...
 *
 * @details
 * These expansions must run before --taskflow-conversion, because that pass
...
struct ExpandMathToArithPass
    : public PassWrapper<ExpandMathToArithPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(ExpandMathToArithPass)

  StringRef getArgument() const override { return "expand-math-to-arith"; }
  StringRef getDescription() const override {
    return "Expand math.fpowi and math.tanh into arith/math.exp primitives";
  }
...
std::unique_ptr<mlir::Pass> mlir::createExpandMathToArithPass() {
  return std::make_unique<ExpandMathToArithPass>();
}
